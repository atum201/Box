var ChatApp = function(user, socket, option){ // tap trung xu ly, dong bo du lieu,
    ...
    var CHAT_SERVER = option.server || "";
    	AVATAR_DEFAULT = option.avatar||"smart_admin/img/avatars/male.png",
    ...
    var _timeoffline = function(offline) { 
        try {
            ...
            if (timeoff >= 31536000) {// year
                ...
                return parseInt(timeoff / 2592000) + " tháng";
            } else if (timeoff >= 86400) { // day
                return parseInt(timeoff / 86400) + " ngày";
            } else if (timeoff >= 3600) { // hour
                return parseInt(timeoff / 3600) + " giờ";
            } else if (timeoff >= 60) { // minute
                return parseInt(timeoff / 60) + " phút";
            } else { // second
                return parseInt(timeoff) + " giây";
            }
        } catch (exceptions) {
        return "";
        }
    ...
    var _sortContact = function(){
        // Object.keys(_contacts).sort((a, b) => {
        ...
        //     return _sortText(a.nickName,b.nickName);
        // })
    ...
    var ChatBox = function(box, state) {
                ...
                if(_.last(_this.messages) && _.last(_this.messages).from == message.from && indexDate !== -1){ 
                    ...
                    if(message.from == _user.id){
                        ...
                        var d = $("<div class=\""+MESSAGE_RECEIVER+"\">"+
                                    "<div class=\"avatar\">"+
                                        "<img src=\""+_getAvatarPath(_contacts[message.from].avatar)+"\">"+
                                    "</div> </div>");
        ...
        this.init = function () {
            ...
            textinput.focus(function () {
                ...
                _this.box_head.removeClass('shimmer');
            });
            textinput.focus();
    ...
    var ChatContact = function(state, title){
        ...
        this.contact_tab_nav = CHAT_CONTACT_LEVEL ?
                                $("<ul class=\"nav nav-tabs\">"+
                                    ...
                                    "<li><a data-toggle=\"tab\" href=\"#tab-recently\">H&#7897;i tho&#7841;i</a></li>"+
        ...
        this.createContactor = function(id,pc){
            ...
            var avatar = _getAvatarPath(contact.avatar);
        ...
        this.updateContactor = function(id,dom){
            var _this = this;
            var avatar = _getAvatarPath(_contacts[id].avatar);
    ...
    var _callback = function(type,data){
        if(type === CHATCONTACT_CALL_INSTANCE){// call 1 chat box from chatContact by click or dblclick
            ...
            var textinput = _listBox[data.id].box_content_input.find("textarea");
            textinput.focus();
        } else if(type === CHATBOX_UPDATE_CONTACT){
            _user = _.assign(_user,data);
            // _localStorage(STORAGE_USER,_user);
            _chatContact.loadFriend(_user.friends);
        } else if(type === CHATCONTACT_SET_STATE){
            _boxs = _.assign(_boxs,{state:data});
            _localStorage(STORAGE_CURRENT,_boxs);
        } else if (type === CHATBOX_SEND_MESSAGE) { // send 1 message. data = { sender: "", receiver: "", contentmessage: "content" };
            socket.emit(SOCKET_SEND_MESSAGE, data);
        } else if (type === SOCKET_SEND_READ_MESSAGE){
            socket.emit(SOCKET_SEND_READ_MESSAGE,data);
        } else if (type === SOCKET_SEND_DEL_MESSAGE){
            socket.emit(SOCKET_SEND_DEL_MESSAGE,data);
        } else if (type === CHATBOX_CLOSE){
            _showBox();
        }
    };
    ...
    var _receiveMessage = function(message,idtemp) { // message{id: from,toUser,toGroup,content,state,time}.
        ...
        if(message.from === _user.id){ // tin nhan cua this.user
            ...
            _listBox[message.from].box_head.addClass('shimmer');
            ...
            _listBox[message.toGroup].box_head.addClass('shimmer');
        }
    };
    var _showBox = function(boxId,priority){
        var count = priority ? 1 : 0;
        _boxs = {time:new Date().getTime(),item:[],state:_chatContact.getState ? _chatContact.getState() : {} }; // reset this.boxs
    ...
    var _setSocket = function(){
        ...
        socket.on(SOCKET_BROADCAST_DISCONNECT, function(contactId){
            try{
            	...
            	console.log("SOCKET_BROADCAST_DISCONNECT");
                console.log(err);
            }
        });
    ...
    var _getAvatarPath = function(path){
    	if(path){
    		return 'upload/' + path;
    	} else {
    ...
    var _changeTitle = function(count){
    	if(count == 0){
    		var newTitle = htmlTitle;
    	} else {
    		var newTitle = '(' + count + ') ' + htmlTitle;
    	}
        document.title = newTitle;
    }
    
    var _addIfNotExist = function(arr, element){
    	if(!arr.includes(element)){
    		arr.push(element);
    	}
    }
    
    var _removeFromArr = function(array, element){
    ...
    this.init = function(){
        ...
        // }else{
            _loadData(_user.userId,"contacts","online","recently",CHAT_CONTACT_LEVEL ? "phancap": undefined ,"friend","group",function(data){
                ...
                _groups = data.group;
                //_chatContact = new ChatContact(state, _contacts[_user.id] ? _contacts[_user.id].nickName : "");
                _chatContact = new ChatContact(state, "LIST");
                _dom.append(_chatContact.dom);
                _boxs = {time:new Date().getTime(),item:[],state:{full:false,tab:0}};
                // _localStorage(STORAGE_CURRENT,_boxs);
                _setSocket();
